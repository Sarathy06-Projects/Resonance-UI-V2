...
export default function Home() {
  ...
  if (posts.length > 0) {
    posts.forEach((post, index) => {
      ...
      // Inject Featured Designers
      if (index === 1 && activeTab === "foryou") {
        feedWithModules.push(
          <div key="discovery-designers" className="py-8 border-b border-zinc-100 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-900/20">
            ...
            <div className="flex gap-4 overflow-x-auto px-4 sm:px-6 pb-4 no-scrollbar snap-x snap-mandatory">
              {mockUsers.map((user, i) => (
                <div key={i} className="snap-start shrink-0 w-[240px] p-6 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-3xl flex flex-col hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-0.5 transition-all duration-300">
                  ...
                  <Button variant="outline" size="sm" className="w-full rounded-full dark:border-zinc-700 dark:text-zinc-300">
                    Follow
                  </Button>
                </div>
              ))}
      ...
      // Inject Popular Articles
      if (index === 3 && activeTab === "foryou") {
        feedWithModules.push(
            ...
            {[1, 2, 3].map(i => (
              <div key={i} className="animate-pulse flex gap-4 p-6 bg-zinc-50/50 dark:bg-zinc-900/20 rounded-3xl border border-zinc-100 dark:border-zinc-800/50">
                ...
                <div className="flex-1 space-y-3 py-1">
                  ...
                  <div className="h-[200px] bg-zinc-200 dark:bg-zinc-800 rounded-2xl w-full mt-4" />
                </div>
              </div>
            ))}
          </div>
