...
// Bottom sheet, built on the same @base-ui/react Dialog primitive as
// components/ui/dialog.tsx - just anchored to the bottom edge with a
// slide-up transition instead of centered, matching the native mobile
// pattern (comment composer, action menus) used elsewhere in the plan.
  ...
  return (
    <SheetPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "fixed inset-x-0 bottom-0 z-50 flex max-h-[85vh] w-full flex-col gap-3 rounded-t-2xl bg-popover p-4 pb-[max(1rem,var(--safe-bottom))] text-popover-foreground ring-1 ring-foreground/10 duration-150 outline-none data-open:animate-in data-open:slide-in-from-bottom data-closed:animate-out data-closed:slide-out-to-bottom",
          className
        )}
        {...props}
      >
        <div aria-hidden className="mx-auto h-1 w-9 shrink-0 rounded-full bg-zinc-300 dark:bg-zinc-700" />
        {children}
        {showCloseButton && (
