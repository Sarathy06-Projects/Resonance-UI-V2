...
import { MessageCircle, Heart, Repeat2, Bookmark, Share, Pin, FileText, Sparkles, Clock, ArrowRight } from "lucide-react";
...
export function PostCard({ post, isDetailed = false }: PostCardProps) {
  ...
  const { likedPosts, bookmarkedPosts, toggleLike, toggleBookmark } = useDataStore();
  ...
  const linkedArticle = post.linkedArticle ? mockArticles.find(a => a.id === post.linkedArticle) : null;
    ...
    >
      {post.badge && (
        <div className="flex items-center gap-1.5 mb-4 text-xs font-bold uppercase tracking-wider text-zinc-500 ml-15">
          {post.badge === "Pinned" && <Pin className="w-3.5 h-3.5" />}
          {post.badge === "Article" && <FileText className="w-3.5 h-3.5" />}
          {post.badge === "Featured" && <Sparkles className="w-3.5 h-3.5" />}
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[14px] text-zinc-500 dark:text-zinc-400 font-medium shrink-0">· {post.timestamp}</span>
          </div>
          ...
          <div className="flex items-center justify-between max-w-[400px] text-zinc-400 dark:text-zinc-500 mt-2">
            ...
            <button onClick={(e) => handleInteraction(e)} className="flex items-center gap-1.5 group transition-colors hover:text-zinc-700 dark:hover:text-zinc-300" title="Repost">
              <div className="p-2 rounded-full group-hover:bg-zinc-100 dark:group-hover:bg-zinc-800 transition-colors -ml-2">
                <Repeat2 className="w-5 h-5" />
              </div>
              <span className="text-[13px] font-medium">{post.shares}</span>
            </button>
            <button 
              onClick={(e) => handleInteraction(e, () => toggleLike(post.id))} 
            ...
            <button 
              onClick={(e) => handleInteraction(e, () => toggleBookmark(post.id))} 
            ...
            >
              ...
              <span className="text-[13px] font-medium">{post.bookmarks}</span>
            </button>
